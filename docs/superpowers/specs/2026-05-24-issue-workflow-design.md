...
在当前实现里，代码托管平台以 GitLab 为主，因此这里用 `MR` 表示 `PR/MR`。

### Main MR Workflow

```mermaid
flowchart TD
    A[Draft Plan\n草拟方案] --> B[Awaiting Plan Confirm\n待确认]
    B --> C[Approved For Dev\n可开发]
    C --> D[In Dev\n开发中]
    D --> E[Verifying\n验证中]
    E --> F[Done\n完成]
    B -. 方案需调整 .-> A
...
### MR State Definitions

| 状态 | 含义 | 进入条件 | 退出条件 |
| --- | --- | --- | --- |
| `Draft Plan` | 机器人或开发者正在整理实现方案 | issue 进入可推进阶段并开始产出方案 | 方案可评审后进入待确认 |
| `Awaiting Plan Confirm` | 方案已生成，等待人确认 | 方案初稿已完成 | 确认通过进入开发，或退回重写 |
| `Approved For Dev` | 方案已确认，可正式实现 | 关键范围、验收口径和风险已确认 | 开始编码实现 |
| `In Dev` | 已进入实现阶段 | 分支/MR 已建立且开始提交代码 | 进入验证阶段 |
| `Verifying` | 做 CI、评审、验收和回归检查 | 代码已可验证 | 通过后完成；不通过则退回开发 |
| `Done` | MR 生命周期结束 | 验证通过且满足合并条件 | 一般不再退出 |

### MR Events And Transitions

| 事件 | 当前状态 | 下一状态 | 说明 |
| --- | --- | --- | --- |
| `plan_ready` | `Draft Plan` | `Awaiting Plan Confirm` | 方案产出完成 |
| `confirm_plan` | `Awaiting Plan Confirm` | `Approved For Dev` | 方案确认通过 |
| `implement_started` | `Approved For Dev` | `In Dev` | 开发启动 |
| `verify_started` | `In Dev` | `Verifying` | 提交验证/评审 |
| `verify_passed` | `Verifying` | `Done` | 验证通过 |

补充说明：

1. 以上事件与当前仓库 `mr_state_machine` 一一对应，是当前可落地的最小闭环。
2. `Awaiting Plan Confirm -> Draft Plan` 和 `Verifying -> In Dev` 是推荐回退路径，建议后续补充显式事件（如 `plan_rework_requested`、`verify_failed`）。
3. 在未补充回退事件前，可通过备注或标签记录回退原因，避免状态语义丢失。

### Issue-MR Coordination Rules

建议联动规则：

1. issue 到 `Planned` 后，MR 才允许进入 `Draft Plan`。
2. MR 到 `In Dev` 时，issue 应保持在 `In Progress`。
3. MR 到 `Verifying` 时，issue 可进入 `In Review`。
4. MR 到 `Done` 后，issue 才可进入 `Done`（或按规则关闭）。
5. 若 MR 验证失败退回开发，issue 也应从 `In Review` 回到 `In Progress`。
